...
\]
that pulls back to $\alpha_{(2)}$ along an embedding $\sfH \hookrightarrow \Bar{\gl}_\infty^{(2)}$ that factors the canonical inclusion $\sfH \hookrightarrow \End(\sfH)$ sending an element $a$ to left multiplication by~$a$.

Here is a cheeky and inexplicit way to define $\Bar{\gl}_\infty^{(2)}$: take the smallest sub-associative algebra of $\End(\sfH)$ that contains both $\sfH$ and $\gl_\infty^{(2)}$ (which is simply the direct sum of the homs between basis elements). 
...
\begin{itemize}
\item Let $E^0_{ab, cd}$ denote the matrix sending $z_1^a z_2^b$ to $z_1^c z_2^d$.
\item Let $E^+_{ab, cd}$ denote the matrix sending $z_1^a z_2^b$ to $z_1^{-c} z_2^{-d} \omega$.
\item Let $E^-_{ab, cd}$ denote the matrix sending $z_1^{-a} z_2^{-b} \omega$ to $z_1^c z_2^d $.
\end{itemize}
Note that the superscript indicates whether the matrix has degree 0, +1, or -1, respectively.
