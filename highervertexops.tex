...
and Zeng works with polynomial functions and forms within it:
\beqn
\Omega^{0,\bu}_b (S^3) = \big(\C[w_i,\Bar{w}_i, \xi_i] \slash (w_i \Bar{w}_i = 1, w_i \xi_i = 0) \big) .
\eeqn
with differential $\dbar_b = \xi_i \del_{\Bar{w}_i}$.
...
In fact, the key result is the following.

\begin{prop}
There is a $SU(2)$-equivariant isomorphism of commutative dg algebras
\beqn
...
The algebra $\sfR$ is naturally an $SL(2)$ representation where the $(z_1,z_2)$ transform in the fundamental representation and $(\lambda_1,\lambda_2)$ transforms in the dual (or contragredient) representation.
Explicitly, the lowering and raising operators are
\beqn
...
An example of a harmonic polynomial is $z_1^a \lambda_2^b$.
A direct computation shows that there is a direct sum decomposition of representations
\begin{align*}
...
\[
q: \C[z_i,\lambda_i] \to R = \C[z_i,\lambda_i]/(\C[z_i,\lambda_i] = 1)
\]
is $SL(2)$-equivariant.
Moreover, inside $V_{a,b}$, any multiple of $|(z,\lambda)|^2$ is annihilated by $q$,
so $R$ is spanned by the images of $H_{a,b}$.
Thus, there is a direct sum decomposition $\sfR = \oplus \sfR_{a,b}$ where $\sfR_{a,b} = q(H_{a,b})$ is the image of the subspace of harmonic polynomials in $\C[z_i,\lambda_i]$ under the quotient map.
...
It maps $\sfR_{a,b}$ to $\sfR_{a+1,b-1} \omega$.
...
Let $\sfH \define H^\bu(\sfA)$ and let $i,p$ be the $SU(2)$-equivariant inclusions and projections $i \colon \sfH_2 \hookrightarrow \sfA_2$, $p \colon \sfA_2 \to \sfH_2$.
