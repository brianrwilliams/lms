...
In \cite{CostelloScheimbauer} a parallel model for the cohomology of the holomorphic $\PDisc_d$ colored operad was proposed; though the exact axioms were not written down.
In recent work, similar models to the one given in \cite{CostelloScheimbauer} have appeared using so-called (multivariable) `$\lambda$-brackets' \cite{BGKWY,GKW}. This work uses such an algebraic structure to give a beautiful proposal for computing Feynman diagrams in holomorphic (and mixed holomorphic-topological) field theories (see the next section for more on renormalization within the context of holomorphic QFT).
Such models are similar in spirit to Lie conformal algebras which use the notion of single-variable $\lambda$-bracket to encode the `singular part' of the data of a vertex algebra.
...
In this subsection we discuss some global aspects of holomorphic factorization algebras.

A map $f \colon X \to Y$ induces a pushforward between categories of factorization algebras.
If $\cF$ is a factorization algebra on $X$, then the pushforward $f_* \cF$ is the factorization algebra on $Y$ which assigns to an open set $U \subset Y$ the value $\cF(f^{-1}(U))$.
Suppose we are in the situation where $\cF$ is the factorization algebra of observables of some quantum field theory on $X$.
In the case that $f$ is a smooth map with compact fibers, then $f_* \cF$ is the factorization algebra of observables for the \textit{compactification} of the quantum field theory along $f$.
In the case $f \colon Z \times Y \to Y$ is the projection, one says that $f_* \cF$ is the compactification of $\cF$ along $Z$.

Another special case of this is when $f$ is the map $X \to \star$.
Then $f_* \cF = \int_X \cF$ is the global sections of the factorization algebra $\cF$ along $X$.
In the case that $\cF$ is a locally constant factorization algebra this agrees with the factorization homology of $\cF$ along $X$.
In the case that $\cF$ is a holomorphic factorization algebra on a Riemann surface $\Sigma$, the object $\int_\Sigma \cF$ is closely related to the chiral homology (or conformal blocks) of the chiral (or vertex) algebra associated to $\cF$ along $\Sigma$.

In this way, when $X$ is a general complex manifold and $\cF$ is a holomorphic factorization algebra, then $\int_X \cF$ is a natural higher-dimensional generalization of the notion of conformal blocks of $\cF$.

For complex manifolds, and holomorphic factorization algebras on them, we consider an explicit example that generalizes a well-known concept in complex dimension one.
First, consider a \textit{real} one-dimensional \textit{topological} factorization algebra $\cA$.
If we identify $\cA$ with the associative algebra that it determines, then the factorization homology $\int_{S^1} \cA$ along the circle agrees with the Hochschild cohomology of the associative algebra $\cA$.
In other words, it is the home for all of the `traces' which the algebra $\cA$ admits.
It is well-known that the elliptic curve, or genus one, conformal blocks of a vertex algebra $\mathbb{V}$ is the natural `home' for the characters of the vertex algebra.
The most basic character of a conformal vertex algebra is the $q$-character defined in terms of a trace:
\beqn
\op{char}_{\mathbb{V}} (q) = \op{Tr}_{\mathbb{V}} (q^{L_0-c/24}) ,
\eeqn
where $L_0$ denotes the action of the vector field $z \del_z$.
The shift by the central charge guarantees that for $\mathbb{V}$ conformal that this expression has nice modularity properties.
...
Indeed, suppose we know that $\mathbb{V}$ corresponds to a holomorphic factorization algebra $\cF_{\mathbb{V}}$ on $\C$.
...
\beqn
\int_{E_q} \cF_{\mathbb{V}} = \int_{S^1} r_* \cF_{\mathbb{V}} = \op{Hoch}(r_* \cF_{\mathbb{V}}) .
\eeqn
For more on this perspective see \cite{GuiLi}.
From this point of view we hope it is clear how the factorization homology along an elliptic curve encodes characters in the sense of vertex algebras.

In higher dimensions we can build a similar picture.
The simplest so-called \textit{Hopf manifolds} are complex manifolds of the form
\beqn
...
\beqn
\int_{H_{\mathbf{q}}} \cF \simeq \op{Hoch}(r_* \cF)
\eeqn
...
In \cite{SWchar}, we have shown how an important invariant called the \textit{supersymmetric index} (or supersymmetric partition function) of a supersymmetric theory along a sphere $S^{2n-1}$ be cast into this framework at the level the holomorphic twist.
...
In the case that $\lie{g}$ is simple (so that the first diagram does not contribute to the anomaly) there is an easy way to produce a non-anomalous holomorphic QFT: take $V$ to be a symplectic representation of the gauge Lie algebra $V = Q \oplus Q^*$, where $Q$ is any $\lie{g}$-representation.
