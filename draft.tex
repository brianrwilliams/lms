...
\]
and hence resembles the chiral sector of the scalar, which we just discussed.
\end{rmk}
...
We were driven, so far, by an example which started from the same data as in the non-holomorphic---the data of a symplectic vector space $V$.
For another very natural example, one can consider a vector space $U$ which is equipped with a \textit{symmetric} pairing $\<-,-\>_U$.
Then, the loop space $L(U^*)$ is equipped with a bivector
\beqn
(f,g) \mapsto \underset{z=0}{\text{Res}} \, \<f, \del g\>_U .
\eeqn
Notice that this bivector does \textit{not} correspond to a symplectic form, but nevertheless we can contemplate the deformation quantization of the $\infty$-dimensional Poisson space $L(U^*)$---or, more invariantly, the space of one-forms on the formal disk with values in $U^*$.
The quantization of this Poisson algebra is the algebra with linear generators $f,g \in U((z))$ satisfying the relation
\beqn
[f,g] = \<f, \del g\>_U .
\eeqn
In the case $U=\C$, this is the associative algebra underlying the free chiral boson CFT.
...
The main idea is that we want to replace the algebra of Laurent series of a single variable with an appropriate algebra involving more than one variable.
...
Renormalization is a key ingredient in Riemannian or Lorentzian quantum field theories.
...
\subsection{Heat kernel renormalization for holomorphic field theories}
