...
Thus, using the quantization of holomorphic field theories 
...
In each diagram at hand, this local functional depends only on the holomorphic gauge field~$A$ is of the form
\beqn
c \int \op{Tr}_M (A \del A \del A)
\eeqn
where $c$ is some nonzero constant and $M$ is some representation of $\lie{g}$ (the value of $c$ is the same for both diagrams, in this example).
Specifically:
\begin{itemize}
\item In the first diagram the representation $M$ is the adjoint representation. 
So, this expression vanishes when $\lie{g}$ is simple, e.g., for $\lie{g} = \lie{sl}(n)$.
\item In the second diagram the representation is $M = V$ (the representation which labels the $\beta\gamma$ system).
\end{itemize}
Thus, the total anomaly is $c \int \left(\op{Tr}_M (A \del A \del A) + \op{Tr}_V(A \del A \del A)\right)$.

These anomalies of a holomorphic gauge theory on $\CC^2$ are holomorphic avatars of the Adler--Bell--Jackiw, or chiral, anomaly in ordinary gauge theory on $\RR^4$ \cite{??}.
...
\owen{Owen's comments that I've tried to address:
\begin{itemize}
\item There isn't a $K$-edge on the wheel?
\item Should we just write out the formulas? What we write is kind of hard to parse, as it is shorthand. I can imagine a person asking "proportional ... how?".
\end{itemize}
}
