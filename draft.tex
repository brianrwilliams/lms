...
In perturbative field theory, which describes the formal neighborhood of the theory at points in such spaces, the fields $\Phi$ are typically described by the smooth sections of a vector bundle on spacetime.
The equations of motion which dictate the dynamics of the particular model are encoded by a series of partial differential equations of the form 
\[
...
These equations of motion are natural to the Riemannian geometry of the manifold $M$, and can be derived from the Euler--Lagrange equations of the simple action functional $\int_M \phi \Delta \phi \, \op{dvol}$.
...
For example, in \cite{semichiral} it is shown that on $X = \C^2$ the quantization of holomorphic BF theory is almost topological.\footnote{More precisely, the divergence-free part of the stress tensor is rendered cohomologically trivial to first-order in perturbation theory.}
...
This class of holomorphic field theories are simple higher-dimensional generalizations of the well-studied `$\beta-\gamma$ system' which appears in: superstring theory~\cite{Polchinski}, as the twist (see below)of the $\cN=(2,0)$ supersymmetric $\sigma$-model, and as the chiral part of the scalar field theory on a Riemann surface.

Consider the case that $Y$ is simply $\C$ and suppose that $X$ is $n$-dimensional.
The fields of the model are
\begin{align*}
\gamma & \in \Gamma(X)\\
\beta & \in \Omega^{n,n-1}(X) .
\end{align*}
and the action is
\beqn
\int_\Sigma \beta \wedge \dbar \gamma .
\eeqn
There is a peculiarity of this theory when $n = \dim_\C X > 1$: in the case, there is a linear symmetry by shifting $\beta$ by a $\dbar$-exact form
\beqn
...
Nevertheless, many techniques we have for understanding free field theories can still be applied to this model.

There is the following nonlinear generalization of the $\beta\gamma$ system when $Y$ is an arbitrary complex manifold.
The space of fields consist of pairs $(\gamma, \beta)$ where $\gamma \colon X \to Y$ is a smooth map and 
\[
...
\eeqn 
Solutions to the equations of motion describe, in part, holomorphic maps $X \to Y$, but there is also derived information contained in this model.
A detailed formulation of this theory has been developed in \cite{BWthesis}.
...
There is a holomorphic avatar of this which describes deformations of complex structure which has first appeared in the literature in work related to topological string theory on Calabi--Yau threefolds~\cite{BCOV}.
Because of its relationship to complex structures, the theory is referred to as \textit{Kodaira--Spencer theory}.
It has been mathematically championed in the works of Costello and Li \cite{CLbcov1,CLbcov2,LiThesis} as an approach to the quantum higher genus $B$-model.
In these works, Costello and Li also provide a generalization of the theory that makes sense in any complex dimension; here we focus just on the more familiar three-dimensional version.
...
Forgoing the apparent global issues, we can still apply formal variational calculus to the action to obtain the equation of motion
\beqn
\dbar \mu + \frac12 [\mu,\mu] = 0 ,
\eeqn
where $[-,-]$ is the Schouten--Nijenhuis bracket (which reduces to just the natural bracket when restricted to vector fields).
The reader can recognize this equation as the Maurer--Cartan equation controlling deformations of complex structure $\dbar\rightsquigarrow \dbar + \mu$.

\subsubsection{Holomorphic scalar field theories / $\sigma$-models}
...
The usual scalar field theory can be written as $\int_{\Sigma} \phi \del \dbar \phi$, where $\Sigma$ is a Riemann surface.
...
When we do this, we can express the action functional in terms of the $(1,0)$-form $\alpha \in \Omega^{1,0}(\Sigma)$ as
\beqn\label{eqn:scalar1}
\int \alpha \dbar \del^{-1} \alpha .
\eeqn
...
The fields are certain $(n+1,n)$-forms $\alpha \in \Omega^{n+1,n}(X)$ which satisfy the constraint
\beqn
\del \alpha = 0.
\eeqn
Note that when $n=0$, so $X = \Sigma$ is simply a Riemann surface, this constraint is automatic.
The action functional is the same \eqref{eqn:scalar1}.
When $n=1$ this is identical to the kinetic part of the Kodaira--Spencer action.\footnote{Indeed, when $X$ is a Calabi--Yau three-fold there is an isomorphism $\Omega^{2,1}(X) \simeq \Omega^{0,1}(X, \T_X)$.}
Generally, this theory is the holomorphic part of a higher-form scalar theory, with action
\beqn
\int_X \|C\|^2 \op{dvol} = \int_X C \d \star \d C , \quad C \in \Omega^{2n+1}(X) .
\eeqn
...
The cocycle which defines the $L_\infty$ central extension is
\[
\label{fhk cocycle}
\begin{array}{cccc}
...
If we evaluate $A^{\lie{g}}_{d}$ on a punctured disk rather than a disk then we obtain a complex which receives a dense embedding from the complex
\beqn
...
At the level of the punctured disk this corresponds to the higher Kac--Moody algebra which is the central extension of the sphere algebra $\lie{\fg}^\bu_{d}$.
...
\end{itemize}
%In fact, more generally, the double dual of any supersymmetric QCD with mesons returns the same theory (up to equivalence). 
...
\beqn\label{eqn:electricflavor}
 \frac{N}{16 \pi^2} \int \op{tr}_{\underline{F}} \left(\sfa \, \partial \sfa \, \partial \sfa\right) 
\eeqn
...
\subsubsection{The special case of $SU(2)$ SQCD}
...
\beqn
(Q, Q^\dagger) \in \Omega^{0,\bu}(\CC^2, V_Q^{\oplus 2}) .
\eeqn
...
\beqn
V_{\check{M}} = (\CC^F)^* \otimes \CC^F \cong \CC^{\oplus F^2} .
\eeqn
...
\beqn
W = \frac16 \Phi \wedge \Phi \wedge \Phi .
\eeqn
...
%A duality 
%
%The basic two-dimensional supersymmetric theories involved in both sides of this duality are particular gauged linear $\sigma$-models (GLSMs) with $\cN=(2,2)$ supersymmetry. 
%The physics terminology for the supersymmetric GLSM is a two-dimensional $\cN=(2,2)$ supersymmetric gauge theory coupled to some number of chiral multiplets transforming in a particular representation.
%For gauge group $U(N)$, one takes $F_f$ chiral multiplets valued in the fundamental representation $\CC^N$ and $F_a$ chiral multiplets valued in the anti-fundamental representation.
