...
Let $V_N$ denote the {\em fundamental} (or defining) representation of $SL(N)$,
so $V_N = \CC^N$ with its natural left action.
...
\[
V_Q = \Hom(\CC^F, V_N) \cong (V_N)^{\oplus F},
\] 
whose fields are
\[
...
\[
V^*_Q = \Hom(V_N, \CC^F) \cong (V_N^*)^{\oplus F}, 
\]
whose fields are
\[
...
Fix a positive integer $F$, for flavor.
There will be two theories. 
We call one side {\em electric} and the other {\em magnetic}.
The electric theory is the holomorphic QCD with $N$ colors and $F$ flavors, {\em without} mesons.
{\it We restrict to $F \geq N \geq 2$.}
The magnetic theory is the holomorphic QCD with $\Tilde{N} = F-N$ colors and $F$ flavors, but with mesons $\Tilde{M}$ where $$\Tilde{V}_M = \End(\CC^F) \cong \CC^{\oplus F^2}$$ and with superpotential $$\Tilde{W} =  \Tr(\Tilde{q} \Tilde{M}  \Tilde{Q}).$$
Again, we require $\Tilde{N} \geq 2$.
