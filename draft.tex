...
It has had a large impact in theoretical physics (with nearly two thousand citations),
...
these reductions are related to work of Hori and Hori--Tong \cite{Hori, HorTon} and their mathematical offshoots \cite{Segal, etc}.
...
\subsubsection{Extending the duality} 

There is a way to define the dual of any supersymmetric QCD theory with mesons that satisfies the following properties:
\begin{itemize}
\item In the case that there are no mesons in the original theory, as in the electric theory, the dual agrees with the magnetic theory.
\item The dual of the magnetic theory is equivalent to the original electric theory. 
In other words, the double dual of the electric theory is equivalent to itself. 
\end{itemize}
%In fact, more generally, the double dual of any supersymmetric QCD with mesons returns the same theory (up to equivalence). 
Let us briefly remark how to state this more general duality in the holomorphic context. 
...
The dual theory should again be a holomorphic QCD theory with mesons:
it is holomorphic QCD with $F-N$ colors, $F$ flavors, and with mesons valued in 
\beqn
...
\eeqn
Notice that when $V_M = 0, W=0$, this construction agrees with the dual magnetic theory that we introduced above. 

Now, let's check how it fits with the second condition we requested. 
Start with the magnetic theory with $F-N$ colors, $F$ flavors, and $F^2$ mesons.
...
This term has the effect of equipping both the mesons $\mu$ and $\check \mu$ with mass, 
and hence these fields can be integrated out to obtain the original electric theory. 
...
We now turn to fleshing out these assertions, but we will only discuss (1) and (2).
Aspect (1) is necessary even to formulate a precise statement,
and aspect (2) uses results --- notably the higher Kac-Moody algebras --- that we've already encountered in this survey.
Spacetime symmetries involve similar tools and ideas, albeit in a modified form, much like similarities between studying the Virasoro algebra and affine Lie algebras.
...
We call it the {\em moduli stack of classical vacuum solutions} for the theory~$\cT$.
...
For example, the holomorphic twist of pure $\cN =1$ Yang-Mills theory for group $G$ (i.e., the vector multiplet) has
\[
\cM_{\rm vac} \cong [\fg^*/G],
\]
the coadjoint quotient stack of the group.
(We are working in a $\ZZ/2$-graded world here.)
When $G$ is semisimple and using the invariant pairing on $\fg$, 
we can further identify this with the adjoint quotient stack~$[\fg / G]$. 
...
Instead, they have worked with a variety arising as a GIT quotient.
...
The physicists' focus on the moduli space is quite reasonable:
one must make measurements --- i.e., evaluate functions --- to identify the state of a physical system.
The moduli space captures what can be seen by measurements (here, the ``vacuum expectation values'' of different vacua).
In other words, the possible classical states --- viewed as multiplicative linear functionals on the algebra of observables --- are parametrized by an affine scheme underlying the derived moduli stack.
...
By contrast, the space cannot distinguish between some vacua because it collapses orbits.
...
\]
By contrast the moduli space $M_{\rm vac}$ analyzed by physicists is a variety already well-studied in classical algebraic geometry.
We will now discuss the moduli spaces appearing in the physics literature on Seiberg duality.
(There are some subtleties in relating the moduli spaces of the supersymmetric theories to the moduli stacks of their twists;
we will not dwell on them here, but they need to be resolved to obtain a complete story.)
...
This relationship, which holds for the supersymmetric theory before twisting, was one of the original pieces of evidence offered by Seiberg for his duality.
...
We recalled in Section~\ref{subsec: FHK} an algebraic model $\sfA_2$ for the dg algebra $\Omega^{0,\bu}(\C^2 - 0)$.
...
(In terms of Section~\ref{subsec: FHK}, these determine higher Kac--Moody algebras $\Hat{\sl(F)}_{2,\theta_{\sfE}}, \Hat{\sl(F)}_{2,\theta_{\sfM}}$ when studying the theories on the punctured space~$\CC^2 - 0$.)
...
These anomlies agree: they are labeled by the same cocycle 
\[
\theta_\sfE = \theta_\sfM.
\] 
...
This result tells us that, over the stable locus, the observables of a holomorphic QCD theory agree with a sheaf of two-dimensional chiral differential operators, as discussed in Section~\ref{sec: higher CDO}.
...
One motivation for these suggestions comes from work on two-dimensional (0,2)-theories, such as \cite{DeduGuk, EagLocSha}.
...
\beqn
{\rm Map}\left(\RR^2, T^*[1] \left[ \C^{NF} / GL(N)\right]\right) .
\eeqn
