...
The holomorphic supercharges breaks the $SO(4;\C)$ symmetry down to the group $MU(2)$ which is a double cover of $U(2)$.
...
We assume that the $R$-charges of the fermions in the quarks and anti-quark multiplets is the same.
If this $R$-charge is $r$ then the twisted quarks and anti-quarks take the form
\[
Q \in \Omega^{0,\bu}(\CC^2 , K_{\CC^2}^{(r+1)/2} \otimes V_Q),\quad Q^\dagger \in \Omega^{0,\bu}(\CC^2 , K_{\CC^2}^{(r+1)/2} \otimes V_Q^*)
\]
where $V_Q \cong \C^{NF}$.
...
This anomaly comes from a one-loop diagram with a single vertex labeled by $\Delta^{hol}$, and leads to the constraint that
\beqn
N + F r = 0 .
\eeqn
In other words, $r = - N / F$. 
This is the exact condition one must require for $R$-symmetry to be anomaly free in the untwisted theory. 
On the magnetic side the condition becomes $r = N/F - 1$.
...
\subsection{Two-dimensional TFTs by reduction}
...
Let's first consider the electric side which breaks up into holomorphic BF theory coupled to a complex two-dimensional $\beta\gamma$ system.
...
This is the space whose algebra of functions is $\cO(\C^{NF}) ((z))((w))$ where $z,w$ are formal loop parameters which we imagine as describing the winding of the two circles we have compactified. 
...
Similarly, the dimensional reduction of holomorphic BF theory is topological BF theory (which is a stacky version of the B-model whose target is $B GL(N)$).
...
As a derived mapping space, this model is given by
\beqn
...
\begin{itemize} 
\item (electric) the $E_2$ algebra of polyvector fields on the stack $\C^{NF} / GL(N)$.
\item (magnetic) the $E_2$ algebra which results from deforming the $E_2$ algebra of polyvector fields on the stack $\C^{(F-N)F} / GL(F-N) \times \C^{F^2}$ by the differential $\{\check{W},-\}$ where $\{-,-\}$ is the Schouten bracket.
\end{itemize}
\end{conj}
...
\begin{dfn}
Let {\em moduli space of quantum vacua} $M^q_{\rm vac}(F,N)$ mean the global horizontal sections over $\cM_{\rm vac}(F,N)$ of this sheaf. 
