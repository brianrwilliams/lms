...
The {\em magnetic theory} $\cT_{\sfM}(F, \check{N})$ is the holomorphic QCD with $\check{N}$ colors and $F$ flavors, but with mesons $\check{M}$ where $$V_{\check{M}} = \End(\CC^F) \cong \CC^{\oplus F^2}$$
...
The superpotential is $$\check{W} =  \Tr(\check{Q}^\dag \check{M}  \check{Q}),$$
...
But this feature also makes the conjecture powerful.
...
We also mention that we restrict the values of $N$ and $\check{N}$ as above to focus on the simplest cases of duality;
there are more subtle versions for cases like $F = N+1$.
\owen{Fair?}
...
the derived stack of classical vacua is 
\[
...
The image thus satisfies both sets of Pl\"ucker relations, the condition that the composite $YX$ has rank at most $N$, and some further relations, given by the Second Fundamental Theorem.
In other words,
\[
...
One disocvers the need for mesons and the superpotential in order to obtain this result.
