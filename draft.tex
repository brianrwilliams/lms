...
\dedicatory{To Yuri Manin, with great admiration.}
...
When $X$ is a Calabi--Yau three-fold and $\fg$ is equipped with an invariant pairing, 
...
\owen{Mention main classes of field theories (scalar and sigma models, fermions as matter, gauge theories for ``forces'', gravity, and their combinations) and how there are holomorphic analogs}
...
\subsubsection{Holomorphic fermionic field theories}
...
\subsection{Examples from supersymmetric field theories}
...
\owen{Revise to explain SYM as source for some examples above, but also add brief discussions of newer examples like twisted SUGRA and exotic super Lie algebras}
...
\section{Holomorphic $\sigma$-models, higher algebras of differential operators, and free field realization}
...
The field content of 4d $\cN=1$ supersymmetric Yang--Mills theory is:
\begin{itemize}
...
From hereon we fix a nonzero, square zero supercharge of the 4d $\cN=1$ supersymmetry algebra for which we wish to twist by. 
The twist of 4d $\cN=1$ supersymmetric Yang--Mills theory has been characterized in \cite{CosYangian, ESW, SWchar}. 
...
Following physics terminology, will refer to the defining $N$-dimensional representation $\CC^N$ of $\SU(N)$ as the {\em fundamental} representation. 
Also, suppose $N_c, N_f$ are integers with $N_f \geq N_c \geq 2$.
...
%\begin{dfn}
%Suppose $\cT_A (N_c, N_f, N_{\rm mes}, W)$ is a type A holomorphic QCD. 
%Define the {\em dual} type A holomorphic QCD to be
%\[
%\Hat{\cT}_{A} (N_c, N_f, N_{\rm mes}, W) \define \cT_{A} (N_f - N_c, N_f, N_{\rm mes} + N_{f}^2 , W + 
%\]
%\end{dfn}
...
\begin{thm}
Both $\cT_{\sfE}(N_c, N_f)$ and $\cT_{\sfM}(N_c, N_f)$ exist at the quantum level. 
...
N_c \frac16 \int \op{tr}_{N_f} \left(\sfa \partial \sfa \partial \sfa\right) 
\eeqn
...
We start off with the (complexified) 4d $\cN=1$ superconformal algebra $\sl(4|1)$.
This super Lie algebra is the defining symmetry of a 4d $\cN=1$ superconformal field theory. 
...
\begin{thm}[\cite{SWsuco}] \label{thm:swsuco}
