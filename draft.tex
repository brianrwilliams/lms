...
this family $w_{\gamma,t}$ has a natural smooth extension to the Schwinger space~$\Schw(\gamma)$.
That is, there is a smooth map
\[
w_{\gamma}: \Schw(\gamma) \to \Hom_{cts}(\cV_c^{L(\gamma)},\CC)
\]
where $w_{\gamma,t}$ is a continuous linear functional on $\cV_c^{L(\gamma)}$ for every point $t \in \Schw(\gamma)$.it 
\end{thm}
